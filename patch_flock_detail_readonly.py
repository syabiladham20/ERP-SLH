    ...
    target_str = """              onClick: (e, elements) => {
                  if (elements.length) {
                      ...
                      if (notePoint) {
                          ...
                          if (rawData && rawData.note) {
                              // Use the rawData.date we saved in the point, not a calculated one
                              showNoteModal(rawData.date, rawData.note, rawData.photos || rawData.photo);
                          }
                      }
                  }
              }"""
    ...
    replacement_str = """              onClick: (e, elements) => {
                  ...
                  if (elements.length) {
                      ...
                      if (notePoint) {
                          ...
                          if (rawData && typeof rawData === 'object' && rawData.note) {
                              // Use the rawData.date we saved in the point, not a calculated one
                              showNoteModal(rawData.date, rawData.note, rawData.photos || rawData.photo);
                          }
                      }
...
patch_file('templates/flock_detail_readonly.html')
