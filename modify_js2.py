...
const floatingNotePlugin = {
    ...
    afterEvent(chart, args) {
        if (args.event.type === 'click') {
            ...
            // Check if clicking existing note (handled by plugin click instead)
            // But just in case, don't trigger add note if they clicked near an annotation?
            // Chartjs-plugin-annotation click triggers before chart click if element is clicked.
...
for filename in files_to_modify:
    ...
    # remove old code block if exists
    if "// --- Floating Notes Logic ---" in content:
        content = re.sub(r'// --- Floating Notes Logic ---.*</script>\s*{% endblock %}', replacement_js + "\n</script>\n{% endblock %}", content, flags=re.DOTALL)
    else:
