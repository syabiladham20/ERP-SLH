...
def patch_file(filepath):
    ...
    target_str = """          onClick: (e, elements) => {
              if (elements.length) {
                  // Find the index of the Clinical Notes dataset
                  const clinicalNotesDatasetIndex = generalChart.data.datasets.findIndex(ds => ds.label === 'Clinical Notes');

                  // Check if any of the clicked elements belong to the Clinical Notes dataset
...
patch_file('templates/flock_detail_modern.html')
