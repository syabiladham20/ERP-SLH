...
from app.models.models import House, Flock, SamplingEvent, User, Farm

class SamplingPerformanceTestCase(unittest.TestCase):
    def setUp(self):
        ...
        for f in flocks:
            for j in range(5):
                s = SamplingEvent(flock_id=f.id, age_week=j, test_type=f'Type{j}')
                db.session.add(s)
        db.session.commit()
    ...
    def test_sampling_performance(self):
        ...
        for _ in range(10):
            response = self.app.get('/health_log/sampling')
        end_time = time.time()
